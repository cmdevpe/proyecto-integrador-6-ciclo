<div class="mt-14">
                                            ...
                                            <ul class="p-1">
                                                <li>
                                                    <button type="button"
                                                        ...
                                                        @click.prevent="$wire.edit({{ $color->id }}); open = false">Editar</button>
                                                </li>
                                            </ul>
                                            <div class="p-1">
                                                <button type="button"
                                                    class="w-full text-left block py-2 px-4 rounded-md text-sm text-gray-900 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-100 dark:hover:text-white"
                                                    @click.prevent="Swal.fire({title: '¿Estás seguro?', text: '¡No podrás revertir esto!', icon: 'warning', showCancelButton: true, confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: '¡Sí, eliminar!', cancelButtonText: 'Cancelar'}).then((result) => { if (result.isConfirmed) { $wire.delete({{ $color->id }}); open = false; }})">
                                                    Eliminar
                                                </button>
                                            </div>
                                        </div>
    ...
    <x-modal name="color-form-modal" maxWidth="lg" :static="true">
        <x-slot:header>
            <h3 class="text-xl font-semibold">{{ $form->color_id ? 'Editar color' : 'Crear nuevo color' }}
            </h3>
        </x-slot:header>
        <x-slot:body>
            <form class="space-y-4 md:space-y-5" wire:submit.prevent="save"
                wire:key="color-form-{{ $form->color_id ?? 'new' }}">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        <x-input wire:model.blur="form.name" type="text" name="form.name" id="name"
                            label="{{ __('Nombre') }}" placeholder="{{ __('Ingresa el nombre del color') }}"
                ...
                <div class="flex items-center space-x-2">
                    <x-button type="submit" wireTarget="save" class="px-4 py-2">
                        ...
                        <x-slot:loading>
                            {{ $form->color_id ? 'Actualizando color' : 'Creando nuevo color' }}
                        </x-slot:loading>
                        {{ $form->color_id ? 'Actualizar color' : 'Crear nuevo color' }}
                    </x-button>

                    <x-button class="px-4 py-2" color="light"
                        @click="$dispatch('close-modal', { name: 'color-form-modal' })">
                        Cancelar
                    </x-button>
                </div>
            </form>
